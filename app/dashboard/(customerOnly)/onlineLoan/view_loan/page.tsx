...
try {
    ...
    if (!session?.user?.id) {
      ...
      return <p className="text-center mt-10">You need to log in to view your loans.</p>;
    }
    ...
    return <p className="text-center mt-10">Failed to fetch loans. Please try again later.</p>;
  }

  if (!loans.length) {
    return <p className="text-center mt-10">No loans available or failed to fetch data.</p>;
  }
      ...
      <table className="table-auto w-full border-collapse border border-gray-300">
        <thead>
          <tr className="bg-gray-100">
            ...
            <th className="border border-gray-300 px-4 py-2">Fixed Deposit ID</th>
          </tr>
        </thead>
        <tbody>
          {loans.map((loan) => (
            <tr key={loan.Loan_ID}>
              <td className="border border-gray-300 px-4 py-2">{loan.Loan_ID}</td>
              <td className="border border-gray-300 px-4 py-2">{loan.Amount}</td>
              <td className="border border-gray-300 px-4 py-2">{loan.Interest_Rate}%</td>
              <td className="border border-gray-300 px-4 py-2">{loan.Issued_Date.toISOString()}</td>
